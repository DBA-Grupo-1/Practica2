...
public class Charger extends SuperAgent {
	...
	public void onMessage(ACLMessage msg){
		...
		BlockingQueue<ACLMessage> assignedQueue = null;     // Hasta aquí, coger mensaje y su subject.

		//Según el asunto el mensaje se asigna a una cola u otra.
		switch(subject)
		{
		...
		//Se introduce el mensaje en la cola.
		if(assignedQueue != null){
			try{
				assignedQueue.put(msg);
			}
			catch (InterruptedException e){
				e.printStackTrace();
			}
							...
							switch(subject){
								case SubjectLibrary.ChargerBattery:
									try{
										...
										response.put("ChargerBattery", totalBattery);
										send(ACLMessage.INFORM, msg.getSender(), msg.getProtocol(), null, msg.getReplyWith(), msg.getConversationId(), response);
										
										//Meter mensaje en el log
										addMessageToLog(Log.SENDED, msg.getSender(), msg.getProtocol(), subject, String.valueOf(totalBattery));
									}
								...
								case SubjectLibrary.Charge:
									if(content.has("ID")){
										try{
											...
											response.put("Subject", subject);
											response.put("TotalCharge", totalDroneCharge);
											send(ACLMessage.INFORM, msg.getSender(), msg.getProtocol(), null, msg.getReplyWith(), msg.getConversationId(), response);
											
											//Meter mensaje en el log
											addMessageToLog(Log.SENDED, msg.getSender(), msg.getProtocol(), subject, String.valueOf(totalDroneCharge));
										}
										catch(JSONException e){ e.printStackTrace();}
									}
									else{
								...
								case SubjectLibrary.DetailedCharges:
									if(content.has("ID")){
										try{
											...
											response.put("Subject", subject);
											response.put("Charges", DetailCharge);
											send(ACLMessage.INFORM, msg.getSender(), msg.getProtocol(), null, msg.getReplyWith(), msg.getConversationId(), response);
										}
										catch(JSONException e){ e.printStackTrace();}
									}
	...
	private void onReload(ACLMessage msg) throws RefuseException, NotUnderstoodException, JSONException{
		...
		if (msg.getPerformativeInt() == ACLMessage.REQUEST){
			...
			//send(ACLMessage.NOT_UNDERSTOOD, msg.getSender(), msg.getProtocol(), null, msg.getReplyWith(), msg.getConversationId(), null); En principio al lanzar la excepción no se debe de enviar este mensaje.
	...
	private void onSubscription(ACLMessage msg)throws RefuseException, NotUnderstoodException{
		...
		try {
			if(content.get("Subject").equals(SubjectLibrary.DroneRecharged)){
				//Meter mensaje en el log
				addMessageToLog(Log.RECEIVED, msg.getSender(), msg.getProtocol(), SubjectLibrary.DroneRecharged, "");
				if(subscribers.containsKey(msg.getSender().toString()))
					...
					//Meter mensaje en el log
					addMessageToLog(Log.SENDED, msg.getSender(), msg.getProtocol(), SubjectLibrary.DroneRecharged, "");
				}
	...
	private void sendInformSubscribeReached(AgentID droneRecharged, int levelRecharge){
		...
		try {
			...
			content.put("ID-Drone", droneRecharged.toString());
		...
		for(String name: subscribers.keySet()){
			...
			//Meter mensaje en el log
			String logContent = droneRecharged.name + " was recharged " + String.valueOf(levelRecharge)  + " units.";
			addMessageToLog(Log.SENDED, name, ProtocolLibrary.Subscribe, SubjectLibrary.DroneRecharged, logContent);
